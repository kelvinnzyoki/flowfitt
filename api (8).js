...
async function apiRequest(endpoint, options = {}) {
    ...
    // Only set Content-Type when there is a body — GET/HEAD requests must not carry it
    ...
    try {
        ...
        // Do NOT clearTimeout here — keep the timer running through any 401 refresh + retry
        // so the total request lifecycle is bounded by API_CONFIG.timeout
...
async function handleResponse(response) {
    ...
    let data = {};
    try { if (text) data = JSON.parse(text); } catch (_) { /* non-JSON body, keep {} */ }
...
const WorkoutsAPI = {
    ...
    searchExercises: async (query) => {
        ...
        } catch (_) { /* search endpoint unavailable — fall through to filter endpoint */ }
