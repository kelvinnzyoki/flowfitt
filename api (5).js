...
const WorkoutsAPI = {
    getExercises: async (filters = {}) => {
        ...
        if (res && res.success) {
            const d = res.data;
            // Handle every shape the backend might return
            ...
            else if (Array.isArray(d?.results))      res.data = d.results;
            else {
                // Last resort: log the raw shape so developer can diagnose
                console.warn('[WorkoutsAPI.getExercises] Unknown data shape:', d);
                res.data = [];
            }
        }
        return res;
