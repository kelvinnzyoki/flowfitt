    ...
    show: (message, type = 'info') => {
        ...
        toast.innerHTML = `
            <div class="toast-content">
                ...
                <button onclick="this.closest('.toast').remove()" style="background:none;border:none;color:inherit;cursor:pointer;padding:0 0 0 0.5rem;font-size:1rem;opacity:0.6;line-height:1;" aria-label="Dismiss">✕</button>
            </div>`;
        document.body.appendChild(toast);
        setTimeout(() => toast.classList.add('toast-show'), 50);
        const _toastTimer = setTimeout(() => {
            toast.classList.remove('toast-show');
            setTimeout(() => { if (toast.parentNode) toast.remove(); }, 400);
        }, 6500);
        toast._dismissTimer = _toastTimer;
    },
