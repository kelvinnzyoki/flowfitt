...
// ── WORKOUTS / EXERCISES ──────────────────────────────────────────────────────
// FIX: WorkoutsAPI was missing — breaks workouts.html and workout-session.html
const WorkoutsAPI = {
    // Get all exercises with optional filters (category, difficulty, search, limit)
    getExercises: async (filters = {}) => {
        const params = new URLSearchParams(filters);
        return await apiRequest(`/exercises?${params}`);
    },
    // Get single exercise by ID
    getExerciseById: async (id) => await apiRequest(`/exercises/${id}`),
    // Search exercises by name/keyword
    searchExercises: async (query) => {
        const params = new URLSearchParams({ q: query, limit: 20 });
        return await apiRequest(`/exercises/search?${params}`);
    },
};
