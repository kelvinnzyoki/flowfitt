...
const SubscriptionAPI = {
    ...
    getPlans: async () => {
        // Returns all active subscription plans from the backend
        // Falls back gracefully if endpoint doesn't exist yet
        try {
            const res = await apiRequest('/subscriptions/plans');
            if (res && res.success && Array.isArray(res.data?.plans || res.data)) {
                return { success: true, data: res.data?.plans || res.data };
            }
        } catch (e) {
            console.warn('[SubscriptionAPI.getPlans] API unavailable, will use static fallback:', e.message);
        }
        return { success: false, data: [] };
    },

    createStripeCheckout: async (planIdOrSlug) => await apiRequest('/subscriptions/stripe/checkout', {
        method: 'POST',
        // Send both planId and plan so old + new backends both work
        body: JSON.stringify({
            planId: planIdOrSlug,
            plan: String(planIdOrSlug).replace(/-/g,'').length > 20
                    ? planIdOrSlug           // looks like a UUID — send as-is
                    : String(planIdOrSlug).toUpperCase(),  // short slug → uppercase
            successUrl: window.location.origin + '/subscription.html?success=1',
            cancelUrl:  window.location.origin + '/subscription.html?cancel=1',
        }),
    }),
