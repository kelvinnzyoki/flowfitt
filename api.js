...
const TokenManager = {
    ...
    // refreshToken lives in an httpOnly cookie — never touch it from JS
    setTokens: (accessToken) => {
        localStorage.setItem('accessToken', accessToken);
        // refreshToken is set by the server as an httpOnly cookie automatically
    },
    clearTokens: () => {
        localStorage.removeItem('accessToken');
        localStorage.removeItem('user');
        // The server clears the refreshToken cookie on /auth/logout
    },
    getUser: () => {
...
// When multiple concurrent requests all get TOKEN_EXPIRED simultaneously,
// this ensures only ONE call to POST /auth/refresh is made. All concurrent
// callers await the same promise. Without this, request N+1 would send the
// already-rotated (now-invalid) old cookie and get 401, causing logout.
let _refreshInFlight = null;

async function refreshAccessToken() {
    if (_refreshInFlight) return _refreshInFlight;
    _refreshInFlight = _doTokenRefresh().finally(() => { _refreshInFlight = null; });
...
async function _doTokenRefresh() {
    try {
        ...
        // Guard against non-JSON error responses (e.g. Vercel HTML 5xx pages)
        if (!response.ok) return false;
        const data = await response.json();
        if (data.success && data.data?.accessToken) {
            // Server sets the new refreshToken cookie in the response headers.
...
async function checkAuth() {
    if (isAuthenticated()) {
        try {
            ...
            // Do NOT call TokenManager.clearTokens() here on every error.
            // apiRequest() already handles the TOKEN_EXPIRED → refresh → retry
            // flow and only clears tokens + redirects when the refresh itself
            // fails with a genuine auth error. Clearing here on network errors
            // or 5xx responses would log the user out when their session is
            // still valid.
