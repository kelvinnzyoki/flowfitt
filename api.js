...
async function refreshAccessToken() {
    try {
        ...
        // No need to read it from localStorage or include it in the body.
        ...
        if (data.success && data.data) {
            // Server sets the new refreshToken cookie automatically in the response.
            // We only need to update the accessToken in localStorage.
...
const WorkoutsAPI = {
    getExercises: async (filters = {}) => {
        ...
        // e.g. { category: undefined } → server ignores it correctly
        const clean = Object.fromEntries(
            Object.entries(filters)
                .filter(([, v]) => v !== undefined && v !== null && v !== '')
                .map(([k, v]) => [k, String(v)])   // URLSearchParams requires strings
        );
        const params = new URLSearchParams(clean);
        const res = await apiRequest(`/workouts?${params}`);
        if (res && res.success) {
            // Server returns { success, data: Exercise[], meta: {...} }
            // data is already the array — just ensure it is before returning
            if (!Array.isArray(res.data)) res.data = [];
        }
        return res;
    },
    searchExercises: async (query) => {
        // GET /workouts/search?q= returns { success, data: Exercise[] }
        // Requires q.length >= 2 (server enforces this).
        const r = await apiRequest(`/workouts/search?q=${encodeURIComponent(query)}`);
        if (r && r.success) {
            // data is already the Exercise array — ensure it before returning
            if (!Array.isArray(r.data)) r.data = [];
        }
        return r;
    },
    getExerciseById: async (id) => {
        // GET /workouts/:id returns { success, data: Exercise }
        // data is the Exercise object directly — no nested .exercise key
