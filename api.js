...
const SidebarUser = {
    ...
    async _refreshBadgeFromAPI() {
        try {
            ...
            // Determine the plan slug or tier
            const slug = sub?.plan?.slug || sub?.planSlug || sub?.tier || sub?.type || '';
            
            // Update stored user data with current subscription info
            const user = TokenManager.getUser();
            if (user) {
                user.subscription = slug || status;
                user.subscriptionData = { plan: slug, status: status };
                TokenManager.setUser(user);
            }
            
            // Apply badge based on status priority
            if (['CANCELLED','EXPIRED','INACTIVE','INCOMPLETE_EXPIRED'].includes(status)) {
                ...
                // For active subscriptions, use the plan slug or fall back to 'PRO'
                SidebarUser._applyBadgeFromStr(slug || 'PRO');
            } else if (slug) {
                // Fallback: use slug for any other status
                SidebarUser._applyBadgeFromStr(slug);
            }
