    ...
    try {
        ...
        if (response.status === 401) {
            ...
            if (body.code === 'TOKEN_EXPIRED' || TokenManager.hasSession()) {
                // TOKEN_EXPIRED: access token present but expired — refresh and retry.
                // hasSession() with no code: ff_access cookie also expired (>15 min on
                // the page) but ff_refresh cookie may still be valid (7-day window).
                // In both cases attempt one silent refresh before giving up.
...
// clearTokens() → redirect to /login. User is logged out every 15 minutes.
...
// ── PROGRAMS ──────────────────────────────────────────────────────────────────
// FIX: normalize backend response shapes so /programs does not falsely fall
// back to sample programs when the server returns 200 with { data: { programs } }
// or other wrapped payloads.
function _extractArrayPayload(payload, keys = []) {
...
function _normalizeEnrollment(enrollment) {
    if (!enrollment || typeof enrollment !== 'object') return enrollment;
    return {
        ...enrollment,
        completedDays: Number(enrollment.completedDays ?? enrollment.completed_days ?? 0),
        program: enrollment.program ? _normalizeProgram(enrollment.program) : enrollment.program,
    };
}
...
const ProgramsAPI = {
    getPrograms: async (filters = {}) => {
        ...
        const params = new URLSearchParams(clean);
        const response = await apiRequest(`/programs${params.toString() ? `?${params}` : ''}`);
        const programs = _extractArrayPayload(response, ['programs', 'items', 'results', 'records']);
        return {
            ...response,
            success: response?.success !== false,
            data: programs.map(_normalizeProgram),
        };
    },

    getProgramById: async (id) => {
        const response = await apiRequest(`/programs/${id}`);
        if (response?.data) response.data = _normalizeProgram(response.data);
        return response;
    },

    enrollInProgram:  async (programId)    => await apiRequest(`/programs/${programId}/enroll`, { method: 'POST' }),
    restartProgram:   async (programId)    => await apiRequest(`/programs/${programId}/enroll`, { method: 'POST' }),

    getUserPrograms: async () => {
        const response = await apiRequest('/programs/my-enrollments');
        const enrollments = _extractArrayPayload(response, ['enrollments', 'programEnrollments', 'items', 'results', 'records']);
        return {
            ...response,
            success: response?.success !== false,
            data: enrollments.map(_normalizeEnrollment),
        };
    },
...
const SubscriptionAPI = {
    ...
    // GET /subscriptions/current — returns { subscription: CurrentSubscription | null, plan?: PublicPlan }
    // Normalise to { success, data: CurrentSubscription | null, plan?: PublicPlan }
    getCurrentSubscription: async () => {
        const res = await apiRequest('/subscriptions/current');
        if (res && 'subscription' in res) {
            return {
                success: true,
                data: res.subscription,
                plan: res.plan || res.subscription?.plan || null,
            };
        }
...
const FREE_PLAN_DEFAULTS = {
    slug: 'free', name: 'Free',
    maxWorkoutsPerMonth: 10, maxPrograms: 2,
...
async function fetchActivePlan() {
    const CACHE_KEY = 'ff_plan_cache_v3';
    ...
    try {
        ...
        const freePlan = res?.plan || null;
        const plan = isActive && sub.plan ? sub.plan : (freePlan || FREE_PLAN_DEFAULTS);
        try {
...
function invalidatePlanCache() {
    try { sessionStorage.removeItem('ff_plan_cache_v3'); } catch {}
}
...
function showUpgradePrompt(featureName, minPlan = 'pro') {
    ...
    el.innerHTML = `
        <div style="background:linear-gradient(145deg,#13131a,#1c1c28);border:1px solid rgba(212,175,55,0.35);border-radius:22px;padding:2.5rem 2rem;max-width:400px;width:100%;text-align:center;box-shadow:0 30px 80px rgba(0,0,0,0.7)">
            ...
            <a href="/subscription" style="display:block;width:100%;padding:.95rem;background:linear-gradient(135deg,#D4AF37 0%,#edcf7d 50%,#b8860b 100%);color:#000;font-weight:800;font-family:'Oswald',sans-serif;letter-spacing:2px;border-radius:12px;text-decoration:none;font-size:1rem;margin-bottom:.75rem">
...
// ── ADMIN ─────────────────────────────────────────────────────────────────────
const AdminAPI = {
    summary: async () => await apiRequest('/admin/summary'),
    users: async (params = {}) => await apiRequest(`/admin/users${toQueryString(params)}`),
    subscriptions: async (params = {}) => await apiRequest(`/admin/subscriptions${toQueryString(params)}`),
    workouts: async (params = {}) => await apiRequest(`/admin/workouts${toQueryString(params)}`),
    programs: async (params = {}) => await apiRequest(`/admin/programs${toQueryString(params)}`),
    programEnrollments: async (params = {}) => await apiRequest(`/admin/program-enrollments${toQueryString(params)}`),
    feedback: async (params = {}) => await apiRequest(`/admin/feedback${toQueryString(params)}`),
    activity: async () => await apiRequest('/admin/activity'),
    updateFeedbackStatus: async (id, status) => await apiRequest(`/admin/feedback/${encodeURIComponent(id)}/status`, {
        method: 'PATCH',
